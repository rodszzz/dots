...
local opts = {
    ...
    --other
    menu_timeout = 10,
    --use youtube-dlp to fetch a list of available formats (overrides quality_strings)
    fetch_formats = true,
...
function download_formats()
    ...
    mp.osd_message("fetching available formats with youtube-dlp...", 60)

    if not ytdl.searched then
        local ytdl_mcd = mp.find_config_file("youtube-dlp")
        if not (ytdl_mcd == nil) then
            msg.verbose("found youtube-dlp at: " .. ytdl_mcd)
            ytdl.path = ytdl_mcd
        end
        ytdl.searched = true
    end
    ...
    res = {}
    msg.verbose("youtube-dlp succeeded!")
